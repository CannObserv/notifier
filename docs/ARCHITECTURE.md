...
scripts/                     — Helper scripts: `load_env.sh` (source it — the one env-loading recipe, resolves the repo `.env` from its own location so it works from any cwd), `dev_server.sh` (the only supported dev-server launch — loads secrets, swaps in `DEV_DATABASE_URL`, runs the guard, then uvicorn on 9001), `seed_tenant.py` (provision tenant + API key; production requires the opt-in flag), `dump_openapi.py` (dump app.openapi() JSON for SDK regen)
