...
src/core/db_safety.py        — Production-database guard. `assert_safe_database_url` refuses any database whose name does not end in `_test`/`_dev` unless `NOTIFIER_ALLOW_PROD_DB=1`. Called from `get_database_url()`, the one chokepoint every connection path crosses. `alembic/env.py` is deliberately exempt (it reads os.environ directly); `python -m src.core.db_safety` is the CLI form used by `scripts/dev_server.sh`
...
tests/deploy/                — Drift tests for the deploy surface: `scripts/dev_server.sh` (guard delegation, safe env loading, port 9001, refusal paths, and that no doc has regrown the unguarded recipe) and `deploy/notifier.service` (the opt-in flag is in the unit and in no env file)
...
scripts/                     — Helper scripts: `dev_server.sh` (the only supported dev-server launch — loads secrets, swaps in `DEV_DATABASE_URL`, runs the guard, then uvicorn on 9001), `seed_tenant.py` (provision tenant + API key; production requires the opt-in flag), `dump_openapi.py` (dump app.openapi() JSON for SDK regen)
