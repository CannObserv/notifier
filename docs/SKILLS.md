...
That entry carries an explicit `"timeout": 120` — the hook's constant from [gregoryfoster/skills#259](https://github.com/gregoryfoster/skills/issues/259), one network round trip per vendored repo plus the `git push` that [#293](https://github.com/gregoryfoster/skills/issues/293) added. It was `60` until #71: the kill that budget produces lands *between* the commit and the push, leaving `main` ahead of `origin/main` on this VM alone, where nothing reachable through the GitHub API can see it. **Re-running `install-refresh.sh` will not repair a wrong value** — #259 made preserve beat prescribe, so a timeout already on the entry wins over `--timeout` and the run only reports the disagreement. Edit `.claude/settings.json` by hand; `tests/deploy/test_skills_refresh_hook.py` pins it.
