...
|---|---|---|
| DB migration | `schema.sql` changed | `apply_schema` or `systemctl restart notifier` |
| Service restart | Code change (no auto-reload in prod) | `sudo systemctl restart notifier` |
